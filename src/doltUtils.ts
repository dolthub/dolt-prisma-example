...
export async function doltCommit(prisma: PrismaTransaction, author:string, msg:string) {
  const result = await prisma.$executeRaw`CALL DOLT_COMMIT('--author', ${author}, '-Am', ${msg}) `;
...
export async function printCommitLog(prisma: PrismaTransaction) {
  const res = await prisma.$queryRaw<CommitLog[]>`SELECT commit_hash, committer, message FROM dolt_log ORDER BY date DESC`;
...
export async function getBranch(prisma: PrismaTransaction,branch:string): Promise<Branch[] | undefined>{
  const res = await prisma.$queryRaw<Branch[]>`SELECT name FROM dolt_branches where name = ${branch}`;
  return res;
}

export async function getBranches(prisma: PrismaTransaction): Promise<Branch[] | undefined>{
